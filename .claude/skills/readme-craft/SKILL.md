...
1. Icon (when one exists or can honestly be made) or badges
2. Title
...
flowchart TD
    ...
    classDef clientSide fill:#eef2fd,stroke:#2b5fd9,color:#14315f
    classDef chainSide fill:#e7f5ee,stroke:#1f8a5b,color:#0f3d2a
    class user,action clientSide
    class contract,verifier,done chainSide
```

The hex values in the classDef lines are placeholders: take them from the
project palette collected in step 1 (the soft variant as fill, the accent
as stroke, the deep variant as the text color).
...
- Color the diagram so the colors illustrate the structure: one classDef
  per boundary, layer, or trust domain (3 or 4 classes maximum), fills
  taken from the project palette collected in step 1. Same domain, same
  color, everywhere; never color nodes one by one for decoration.
- Style completely or not at all: every classDef sets fill, stroke, AND
  color (the text color) together. A hardcoded fill with an inherited text
  color is the classic break: unreadable in one of GitHub's two themes.
  Soft light fills with dark text (the palette's soft variants) survive
  both themes.
- Edges and edge labels stay on the default theme; they adapt by
  themselves.
- When color carries meaning, add a one-line legend under the diagram:
  "Blue, browser side; green, on chain."
- Use `sequenceDiagram` when the order of messages is the point (protocols,
  handshakes); `flowchart` when the topology is.
- Verify the render before done, on GitHub preview or mermaid.live, in
  light AND dark theme. A broken or unreadable diagram block is worse than
  no diagram.
...
[ ] The mermaid block renders in both GitHub themes (preview or
    mermaid.live); every classDef sets fill, stroke, and color, hex values
    from the palette
...
flowchart TD
    ...
    classDef {domain_a} fill:{soft hex},stroke:{accent hex},color:{deep hex}
    classDef {domain_b} fill:{soft hex},stroke:{accent hex},color:{deep hex}
    class {nodes of a} {domain_a}
    class {nodes of b} {domain_b}
```
