...
## Appendix D: worked ground truth: the recessed-tray landing system

A second dissected reference, after the glass example: a validated production landing
page, taken apart for HOW it presents elements, never for what it says. Different
register, same discipline: a warm cream field, one orange accent, white cards resting
in recessed "soft" trays, machined rectangular buttons, one eyebrow label system, and
a near-subliminal orbiting ambient layer. Steal the recipes and the numbers; swap
every color and font for the current project's tokens.

### D.1 The typography discipline

- Two families plus a mono: one geometric sans for headings and UI, one softer face
  reserved for descriptions (a dedicated description class), and `ui-monospace` for
  addresses, codes, and data chips. Every text style sets size, line-height, AND
  letter-spacing together; size alone is never enough.
- The weight ceiling is 500 (medium) for every heading. 600 exists only for eyebrow
  labels, 700 only for the wordmark. Hierarchy comes from size and spacing, never
  from bolding.
- Tracking scales with size: display type is tight (a hero at -0.03em; 52px section
  titles at -1.56px, which is -3% of size), 16px body sits near neutral (+0.04px),
  and small text goes slightly positive. Heading leading is about 1.2x the size
  (52/62.4, 28/33.6, 22/26.4); long-form paragraphs run 1.6.
- Fluid type via clamp, not breakpoints alone: a hero at
  `clamp(2.5rem, 1.83rem + 3.33vw, 4.5rem)` with leading 1.05; key paddings and gaps
  clamp the same way.
- ONE eyebrow recipe used identically everywhere: 12px, semibold, uppercase,
  line-height 1, tracking 1.8px (0.15em), muted ink, flanked by two tiny decorative
  marks (6x10 chevron glyphs) or 4px dots. Section openers, step labels, statuses,
  and captions all reuse it verbatim; zero variants.
- Two-line hero pattern: each line its own block span, line one in ink, line two in
  the accent. Headlines get text-balance, paragraphs text-pretty with a measure cap
  in ch (about 58ch).
- Mono as texture: short scrambled mono strings (aria-hidden, select-none) sit INSIDE
  display headlines at 0.8em with +0.02em tracking; addresses render as 12px mono
  chips on field-colored pills.
- Page rhythm: text columns cap at 640px, the hero at 760px, the page at 1060px with
  30px side padding; sections breathe at 160px vertical on desktop, 80px on mobile.

### D.2 Surfaces: the recessed-tray depth model

The common model floats cards on drop shadows. This system CARVES instead; depth
reads as wells and plinths:

- The tray, the signature surface: a muted "soft" panel, radius 16px, padding 20px,
  with an INSET shadow (`inset 0 0 6px 0 rgba(ink, 0.18)`) and no drop shadow.
  Content cards sit inside it.
- Cards: white, radius 8px, padding 20px, shadowless or with a whisper of lift
  (`0 2px 10px rgba(ink, 0.05)` plus a 1px ring at 6% ink). The tray does the depth
  work, not the card.
- Nesting order is law: white card inside soft tray, or a soft inner well inside a
  white card (`inset 0 0 6px 0 rgba(ink, 0.12)`), never soft on soft, never white on
  white, two levels maximum.
- Radii nest downward: 16 tray, 8 card, 10 inner row, 4 chip, full on pills and
  dots. An inner radius is always smaller than its parent's.
- Shadows are tinted with the ink (an rgb like 6,6,18), never pure black, and live
  at 5 to 18 percent alpha. The one floating exception: the nav pill, on a two-layer
  `0 2px 3px rgba(ink,.05), 0 14px 20px rgba(ink,.05)`.
- Hairlines do the separating: 1px SOLID at 10% ink under a card header, 1px DASHED
  at 10% ink between the rows of a list. Solid means boundary, dashed means items.
- Even the 8px progress dots carry the system: inactive dots are soft with a tiny
  inset shadow, the active dot is solid accent. Nothing is flat by accident.
- The pixel is accounted for: 20px of padding above a 1px border becomes 19px so the
  optical rhythm stays exact. Sub-pixel sizes appear where math demands them.

### D.3 Component render recipes

- **The segmented nav pill.** One floating white bar, 56px tall, radius 8px, split
  into cells by 2px field-colored borders (brand cell, menu cell, action cell); the
  only element allowed to float high.
- **The machined button.** Rectangular, radius 0, deliberately against the rounded
  surfaces. Recipe: accent fill, `box-shadow: 0 0 0 1px <deep accent>, inset 0
  1.4px 1px rgba(255,255,255,.12), 0 1.4px 4px rgba(ink,.3)`. The same-hue 1px ring
  plus the thin inner top highlight reads as a physical key. Dark variant: ink fill,
  a #4a4a4a ring, same highlight. Hover is brightness(1.05 to 1.1), never a color
  swap. Min-height 40px, horizontal padding 18 to 22px.
- **Chips and badges.** Radius 4px, a second soft tone as fill, an inset shadow
  (`inset 0 0 5px rgba(ink,.18)`), eyebrow typography inside.
- **The icon plinth.** Any small icon sits centered on a 40 to 44px white rounded-8px
  square with a whisper shadow. Icons never float bare on the field.
- **Data rows.** White rounded-10px rows (padding about 14x12) inside a soft well;
  each row holds a mono chip on a field-colored pill at the left, a literal
  three-dot ellipsis (3px dots), and one small accent icon at the right. Identical
  rows repeated read as a system; one alone would read as decoration.
- **Progression dots.** Equal circles (about 26px) at 10% accent fill, the active or
  final one solid accent: state told by fill, never by size.
- **Stat circles.** Outlined circles, 3px accent border over 15% accent fill, sized
  by importance (76px flanks, 110px center), the value centered at weight 500, the
  label below in the eyebrow style.
- **Image cards.** A background image absolutely filled, ONE accent tint overlay at
  30% opacity, content stacked above. One tint recipe for every image in the piece.
- **The tilted strip.** Sibling image tiles overlapped with small rotations (-18,
  -8, +14 deg) and slightly varied sizes read as a casual pile; each keeps its 8px
  radius and a 2px white ring where tiles meet.
- **The avatar stack.** Overlapped circles at about -12px spacing, each with a 2px
  border in the TRAY color so the overlaps stay crisp on any background.
- **The accordion.** Opens by animating `grid-template-rows: 0fr -> 1fr` (300ms,
  ease-out), never height. The toggle is a 24px ink circle holding two 12x1.5px
  white bars, one rotating 90deg so the plus folds into a minus.
- **The live-status ping.** Two staggered ping halos (the second delayed about 1s)
  behind a solid 8px core dot, beside an eyebrow label.
- **Inline UI punctuation.** A real element (a toggle pill with its knob, a mono
  scramble) drops inline INSIDE a display headline, vertically aligned, as brand
  texture. One per section at most.

### D.4 The ambient motion layer (the auto-moving background)

The signature: the page feels alive at its edges while the content sits still.

- **Offscreen ring geometry.** Giant concentric circles (1000 to 2400px) are
  centered far outside the viewport (for example `left: calc(50% - 2360px)`) so only
  their arcs bleed into the frame; each is a soft band (a soft fill holding an inner
  white-edged disc), never a stroked line.
- **The orbit carousel.** Small white cards (about 76px, radius 8 to 12px, whisper
  shadow, each holding one 20 to 30px icon) ride a huge circle by index:
  `rotate(i * step) translateY(-radius)`, counter-rotated to stay upright, the whole
  field spinning at a near-subliminal 150s per revolution. Solid ambient objects
  turn an order of magnitude slower than gradient drift (the 8 to 20s band stays
  for gradients).
- **Entrance choreography.** Decorative rings slide in on staggered delays (50 /
  125 / 250ms); content reveals on an IntersectionObserver class flip with
  per-child delays stepping 0 / 50 / 100ms.
- **One easing token per job, reused verbatim.** UI transitions: 300ms on
  `cubic-bezier(0.44, 0, 0.56, 1)` (nav, menu links, burger bars). The scroll
  wheel: 600ms on `cubic-bezier(0.16, 1, 0.3, 1)`. Panel crossfades: 500ms.
  The accordion: 300ms ease-out. Four tokens, zero ad-hoc curves.
- **The vertical marquee.** A duplicated column of eyebrow-style items (each flanked
  by two 4px accent dots, separated by dashed hairlines) scrolls upward on a roughly
  30s loop, masked by TWO gradient overlays at once, one horizontal (opaque field to
  15%, clear 32 to 67%, opaque from 85%) and one vertical (opaque to 20%, clear
  middle, opaque from 80%), so the list dissolves at every edge instead of clipping.
- **The scroll-scrubbed wheel.** A sticky stage inside a several-hundred-vh section
  turns scroll into state: numbered 44px chips ride a large circle by
  rotate/translate, the active number sits in a solid accent square, panels
  crossfade at 500ms, and a "01/05" eyebrow plus the dot row restate the state. On
  mobile the same wheel is tap-driven with prev/next buttons instead of scroll.
- **The staggered menu.** Overlay links start at opacity 0 and -16px x and release
  together on a 300ms shared delay; the backdrop dims and blurs the page behind
  (`rgba(ink,.18)` plus a 5px blur).
- Everything above animates transform and opacity only; hovers change brightness or
  color, never layout.

### D.5 What transfers, and what does not

Steal: the recessed-tray depth model, the single eyebrow recipe, the
size/leading/tracking ratios, the machined button, the icon plinth, solid-versus-
dashed hairline semantics, the one-tint image treatment, the offscreen orbit
ambient, the two-axis marquee mask, the grid-rows accordion, the reveal stagger
steps, and the one-easing-per-job token discipline.

Do not steal: the cream, the orange, the fonts, or any page content. A new project
re-derives palette and faces from its own register (the loop's palette gate); this
appendix supplies the presentation language those tokens flow into.
