...
Routing note: when the piece will be produced by Claude Design (a presentation or
promo animation of the project), the process is track B of `SKILL_CLAUDE_DESIGN.md`:
the duration question comes first and is a hard human gate, then a sequential prompt
set (create, then motion and rhythm, then placement, then polish). This file then
supplies the standards, the research sweep, the spec method, and the gate that track
B applies. When the session implements the piece in code itself, this file is the
process end to end.
...
building. For a Claude Design presentation piece, the duration is never assumed,
even when it seems inferable: ask for it and wait (the track B gate in
SKILL_CLAUDE_DESIGN.md). Everything else in this file can proceed without another
question.
