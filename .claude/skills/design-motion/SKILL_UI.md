...
Scope: this workflow hand-builds, and hand-building is for small changes to
existing UI (a component restyle, spacing, adding or fixing states on
screens that already exist). Creating a NEW surface (a frontend, a page or
screen set, a dashboard, a landing page) defaults to the Claude Design loop
in `SKILL_CLAUDE_DESIGN.md`, which reuses this file's framing, grounding,
research, states matrix, and gate; the session hand-builds a new surface end
to end only when the human explicitly asks for it.
